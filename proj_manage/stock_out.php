...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Update inventory
    $update_inventory_sql = "UPDATE inventory SET quantity = quantity - $quantity WHERE id = $item_id";
    if ($conn->query($update_inventory_sql) === TRUE) {
        // Log in history
        $action = 'Stock Out';
        $log_history_sql = "INSERT INTO stock_history (item_id, action, quantity, date) VALUES ($item_id, '$action', $quantity, NOW())";
        $conn->query($log_history_sql);
